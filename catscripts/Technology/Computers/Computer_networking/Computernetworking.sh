...
egrep -i "r(é|e)seau d'ordinateur(s|)|Ethernet|Wi(| |-)fi|wire(d|less) network|wireless card|(wire(d|less)|Ethernet) router|\bLAN(|s)\b|\bWAN(|s)\b|\bWLAN(|s)\b|\bNIC(|s)\b|\bVLAN|DHCP|network administrator|network administration|\bBOOTP|PXE|\bDNS\b|\bDDoS\b|\bTCP\b|\bIPX\b|IP address|OSPF|EIGRP|network interface card|\bAOSS|Intranet|default gateway|subnet|netmask|network cable|i(p|f)config|port forward|DNSSEC|IPSEC" newpages.txt | egrep -iv 'SSH|Telnet|Lan Hikari|Obi( |-)wan|FTP|VoIP|Skype|Bluetooth|Nic Sheff' >> Computernetworking.txt
...
then
  ...
  export CATNAME="Réseau d'ordinateurs"
  $CATEGORIZE
fi
